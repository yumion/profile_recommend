...
folder = 'summer'
value = '夏'
for page in range(375, pages+1):
    print('tag: {tag}, page: {page}'.format(tag=value, page=page))
    url = 'https://photohito.com/tag/{value}/?o=popular-all&p={page}'.format(value=value, page=page)
    ...
    for image_url in image_urls[3::2][:-1]: # 画像とテキストに同じリンクが貼ってあってだぶるため１つ飛ばし.最後のリンクは除く
        time.sleep(random.randint(5, 50)/10) # 高速にアクセスするとサーバーダウンさせる危険あるため時間おく
        # 画像の個別ページへ飛ぶ
        ...
        if img.mode == "RGB":
            # img.show()
            img.save('dataset/{folder}/{cnt:05d}_{season}_like{like}_views{view}.jpg'.format(folder=folder, cnt=cnt,  season=folder, like=num_favo, view=num_view), 'JPEG', quality=100, optimize=True)
        # elif img.mode != "RGB":
            # img = img.convert("RGB")
        cnt += 1
